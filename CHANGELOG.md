...
- **Los 10 eventos**: 5 buenos (curación completa, lluvia de oro 15-40%, +daño 3 combates, −daño recibido 3 combates, revela toda la niebla del nivel) y 5 malos, suavizados a petición (quita vida/oro 5-15%, −daño hecho 2 combates, +daño recibido 2 combates, invoca 1 enemigo cerca ponderado a la inversa de su poder).
- **Duración en combates, no en turnos**: las 4 bendiciones/maldiciones se cuentan igual que los enfriamientos de habilidad (decrementan al acabar cada combate), no como Grito de guerra/Forma Salvaje (que son por turnos).
- Arte de usuario: 2 hojas de sprites (magenta) recortadas a 128×128 con chroma-key + reescalado uniforme, y 11 ilustraciones de ambientación (convertidas a `.jpg`, ~29MB → ~4MB).
- Batería de pruebas headless propia (distribución del sorteo, límites de daño/oro, invocación adyacente y despierta) — ver AGENTS.md.
...
- **Emboscada del Mausoleo 2 protegida de verdad**: nuevo campo `allowDuplicateId` en los triggers — el motor lo ignora, pero el editor de niveles ya no renombra en solitario uno de los dos puntos de emboscada al detectar el ID compartido (`mausoleo2_ambush`), que es intencional.
- **Palanca con tarjeta cómic**: activar la palanca del cementerio ya no muestra el menú de texto plano — usa la misma plantilla visual que un evento de ambientación (imagen a toda tarjeta, con la pregunta y los botones Sí/No en su hueco de pergamino), con una ilustración propia de un brazo accionando el mecanismo. Reutilizable para futuras palancas: si el evento no tiene `image` asignada, sigue funcionando con el menú de siempre.
- **Tooltip de habilidades pasivas**: en el panel de estadísticas del inventario, cada habilidad comprada muestra su descripción real al pasar el ratón (PC) o al tocarla (móvil), en vez de solo el nombre y las estrellas de nivel.
- **Velocidad de juego, ahora de verdad**: el ajuste de velocidad (lenta/normal/rápida) afectaba antes solo a las pausas entre turnos y acciones de la IA, nunca a la propia animación de movimiento — por eso apenas se notaba. Ahora también escala la velocidad de desplazamiento del héroe y los enemigos: lenta = 1500ms de pausa + 255ms por casilla; normal = 1000ms + 170ms (sin cambios); rápida = 200ms de pausa + 60ms por casilla (con un mínimo de 60ms para que no se vea como un salto roto).
- **Slider de escala de interfaz**: rango ampliado de 0.8–1.6 a 0.5–1.6, para poder encoger la interfaz además de agrandarla.
- **Editor de niveles** (artifact aparte, ver historial propio): Mausoleo 1 y Mausoleo 2 recuperados como mapas incluidos en el propio archivo (protegidos contra borrado accidental), con su terreno y marcadores reales sincronizados desde el juego publicado.
...
- **Caja de enemigo**: conjunto a la mitad de ancho, nombre más pequeño y reposicionado, y la barra de vida cambiada por el mismo estilo de píldora roja del héroe (antes usaba una imagen aparte que ya no pegaba con el marco nuevo), también recolocada con precisión de subpíxel.
- **Botón de reiniciar movido al panel de ajustes** (antes arriba a la derecha, ahora como botón de texto junto a idioma/escala/volumen). En su sitio de siempre ya solo están el aviso de combate y, justo debajo, la tabla de iniciativa.
- **Botón "Saltar turno" con el marco nuevo** que se me entregó como imagen, recortado y usado directamente (sin la técnica de pseudo-elementos de antes). El texto sigue saliendo del archivo de idioma (no viene pintado en la imagen), para no perder el soporte de inglés.
- **Animación de muerte del esqueleto**: los 3 últimos fotogramas (el cadáver ya en el suelo) estaban progresivamente más pequeños de lo que debían (hasta un 20% más finos que el resto). Reescalados para que coincidan, anclados al suelo para que no floten.
- Todo el pulido visual ajustado a base de iteraciones guiadas por capturas de referencia antes de tocar el juego de verdad.

## 0.14.2 — activación inmediata en combate, cajas de enemigo apiladas y ajustes
- **Los enemigos ya se activan al entrar en su rango, no al terminar el turno**: antes, aunque caminaras dentro del radio de activación de un esqueleto, no pasaba nada hasta que se te acababan los PA (o pulsabas saltar turno) — parecía que "no reaccionaban". Ahora se comprueba justo después de cada movimiento y de cada ataque: en cuanto entras en rango, el combate se activa ahí mismo, se corta el turno libre y empieza el turno a turno normal, tal como se pidió ("movimiento ilimitado hasta que activas a alguien"). Esto también explica lo que pasaba con los dos primeros esqueletos del cementerio.
- **Cajas de enemigo apiladas verticalmente a la izquierda** (antes intentaban ponerse en fila y con el arte nuevo no quedaban bien). Nombre más pequeño y con truncado de verdad (con "…" si no cabe) en vez de desbordar el marco.
- **Panel del héroe reajustado**: ancho con tope según el tamaño de pantalla (para que no se salga ni choque con el botón de reiniciar), nombre truncado si hace falta, número de PA un poco más pequeño y mejor centrado en su caja.
- **Icono de combate y barra de iniciativa**: más margen para que no se solapen con el botón de reiniciar; la barra de iniciativa ahora tiene un ancho máximo y salta de línea si hay muchos combatientes, en vez de invadir el lado izquierdo de la pantalla.
- Arreglada una prueba headless que fallaba de vez en cuando sin ser un fallo real del juego (el arquero a veces esquiva/bloquea el golpe con 0 de daño, que es un resultado válido — la prueba no lo contemplaba y lo confundía con "no ha hecho nada").
